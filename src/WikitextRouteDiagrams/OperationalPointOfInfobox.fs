...
module OpPointOfInfobox =
    ...
    module Data =
        ...
        let insert title value =
            DataAccess.typedCollectionInsert<OpPointOfInfobox []> collection (toMap title) value

        let query title =
            DataAccess.typedCollectionQuery<OpPointOfInfobox []> collection (toMap title)

        let queryKeys () =
            DataAccess.collectionQueryKeys collection Map.empty
            |> List.map (fun k -> k.["title"])

        let queryAsStrings title =
            DataAccess.collectionQuery collection (toMap title)
    ...
    let findShortName (title: string) =
        ...
        match Templates.findTemplateParameterString templates "Infobox Bahnhof" "Abkürzung" with
    ...
    let private normalizeTextOfLink (link: Link) =
        (Templates.textOfLink link).Replace("&nbsp;", " ")
    ...
    let private findStationName (p: Parameter) =
        ...
        | Composite (_,
            ...
            Some(Templates.linktextOfLink link1 + "-" + Templates.linktextOfLink link2, Templates.linktextOfLink link1)
        | Composite (_, Composite.String (s0) :: Link link :: Composite.String (s1) :: _) when
            onlyChars (s0) && isValidText (s1) -> Some(s0 + " " + normalizeTextOfLink link + " " + s1, Templates.linktextOfLink link)
        | Composite (_, Composite.String (s0) :: Link link :: Composite.String (s1) :: _) when s0.EndsWith("(") && s1 = ")" ->
            Some(s0 + " " + normalizeTextOfLink link + " " + s1, Templates.linktextOfLink link)
        | Composite (_, Composite.String (_) :: Link link :: Composite.String (s) :: _) when isValidText (s) ->
            Some(normalizeTextOfLink link + " " + s, Templates.linktextOfLink link)
        | Composite (_, Composite.Link link1 :: Composite.String ("/") :: [ Composite.Link link2 ]) ->
            Some(
                normalizeTextOfLink link1
                + "/"
                + normalizeTextOfLink link2,
                Templates.linktextOfLink link1
            )
        ...
        | Composite (_, Composite.Link link1 :: Composite.Link link2 :: _) ->
            Some(
                ...
                + " "
                + normalizeTextOfLink link2,
                Templates.linktextOfLink link2
            )
        | Composite (_, Composite.String (s) :: Composite.Link link :: _) when isValidText (s) ->
            Some(s + " " + normalizeTextOfLink link, Templates.linktextOfLink link)
        | Composite (_, cl) ->
            match Templates.getFirstLinkInList cl with
            | Some (link) -> Some(normalizeTextOfLink link, Templates.linktextOfLink link)
            | _ ->
                match cl with
    ...
    let private matchesSymbolType (parameters: seq<Parameter>) =
        parameters
        |> Templates.existsParameterStringValueInList
            BhfSymbolTypes
            (fun s p ->
                s.Contains(p)
    ...
    let private matchesParameterName (parameters: seq<Parameter>) (names: string []) =
        parameters
        |> Templates.forallStringsInParameterStringNameList names (=)
        ...
        try
            ...
            | Composite (_, cl) ->
                ...
                | Composite.Template (n, _, lp) :: _ when (n = "BSkm" || n = "BSkmL") && lp.Length = 2 ->
                    match (Templates.getFirstStringValue lp.[0]), (Templates.getFirstStringValue lp.[1]) with
                    | Some (km), Some (k2) -> [| (parse2float km); (parse2float k2) |]
                    | _ -> [||]
                | Composite.Template (n, _, lp) :: _ when n = "Coordinate" && lp.Length = 6 ->
                    match (lp.[4], Templates.getFirstStringValue lp.[4]) with
                    | Parameter.String ("text", _), Some (km) -> [| (parse2float km) |]
                    | _ -> [||]
                | _ ->
                    Templates.getCompositeStrings cl
                    |> Seq.map
                        (fun s ->
                            match s with
    ...
    let private findSymbols (parameters: List<Parameter>) =
        Templates.getParameterStrings parameters
        |> Seq.map
            (fun s ->
                match s with
    ...
    let private chooseNonEmptyParameter (index: int) (l: Parameter list) =
        ...
        | Parameter.String (n, _) when
            not (System.String.IsNullOrEmpty n) // bypass hints
    ...
    let findStationOfInfobox (t: Template) =
        try
            ...
            | (n, [], l) when matchConditionWithParameters n [| "BS2"; "BS2e" |] 7 5 [| "T1"; "T2" |] l ->
                matchStationAt 4 l
            | (n, [], l) when matchConditionWithParameters n [| "BS2"; "BS2e" |] 5 3 [| "T1" |] l -> matchStationAt 3 l
    ...
    let dump (title: string) (precodedStations: OpPointOfInfobox []) =
        Data.insert title precodedStations |> ignore
