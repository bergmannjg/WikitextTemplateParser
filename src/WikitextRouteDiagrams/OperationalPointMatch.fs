...
module OpPointMatch =
    ...
    let private equalShortnames (sn1: string) (sn2: string) =
        sn1 = sn2
        || sn1.Length = 4 && sn2.StartsWith sn1
        || sn2.Length = 4 && sn1.StartsWith sn2

    let private matchkindOfWkStationWithDbStationPhase2 (wikiStation: WkOpPointOfRoute) (dbStation: DbOpPointOfRoute) =
        if wikiStation.shortname.Length > 0
           && equalShortnames wikiStation.shortname dbStation.KUERZEL then
            if matchStationDistance wikiStation.kms dbStation.km then
                EqualShortNames
            else
                EqualShortNamesNotDistance
        else if wikiStation.shortname.Length > 0
                && dbStation.KUERZEL.Length > 0
                && wikiStation.shortname <> dbStation.KUERZEL then
            MatchKind.Failed
        else if matchStationDistance wikiStation.kms dbStation.km then
