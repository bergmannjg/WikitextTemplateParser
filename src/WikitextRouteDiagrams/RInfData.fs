...
module RInfData =
    ...
    let private ci =
        System.Globalization.CultureInfo("de-DE", false)

    /// dayOfReference should correspond with date of wikitext download
    let private dayOfReference = System.DateTime.Parse("01.12.2020", ci)
                ...
                (fun dict row ->
                    let data =
                        { Component = 0
                          ...
                          ValidityDateStart = row.[8].AsDateTime(ci)
                          ValidityDateEnd = row.[9].AsDateTime(ci) }

                    if data.ValidityDateStart <= dayOfReference
                       && dayOfReference <= data.ValidityDateEnd then
    ...
    let private loadKuerzelArt (name: string) =
        if name.StartsWith "StrUeb" || name.Contains " Strw "then
            ("", "Abzw")
        else
    ...
    let compareDbDataOpDistances (routenr: int) =
        let rinfRoute = loadSeqsOfSoL routenr
        ...
        |> Seq.map
            (fun s ->
                ...
                | Some (opStartIndex), Some (opEndIndex) when abs (opStartIndex - opEndIndex) = 1 ->
                    let opS = dbRoute.[opStartIndex]
                    let opE = dbRoute.[opEndIndex]
