...
let findStation phase (wikiStations: WkOpPointOfRoute []) (dbStation: DbOpPointOfRoute) =
    let res =
        wikiStations
        |> Array.map
            (fun b ->
                if phase = 1 then
                    matchesWkStationWithDbStationPhase1 b dbStation
                else if phase = 2 then
                    matchesWkStationWithDbStationPhase2 b dbStation
                else
                    matchesWkStationWithDbStationPhase3 b dbStation)
        |> Array.choose id

    if res.Length = 0 then
        Failure(dbStation)
    else
        Success(getBestMatch res)
...
let getRestOfWikiStations (results: ResultOfOpPoint []) (wikiStations: WkOpPointOfRoute []) =
    wikiStations
    |> Array.filter
        (fun p ->
            not (
...
let private checkDbDataInWikiDataPhase2
    (route: RouteInfo)
    (wikiStations: WkOpPointOfRoute [])
    ...
    |> Array.map
        (fun r ->
            match r with
            | Success _ -> r
            | Failure p -> findStation 2 restOfWikiStations p)
    |> removeDoubleWkStations

let private checkDbDataInWikiDataPhase3
    (route: RouteInfo)
    (wikiStations: WkOpPointOfRoute [])
    (results: ResultOfOpPoint [])
    =
    ...
    |> mapWithContext
        (fun prev curr next ->
            ...
            | Some (Success (_, _, mk1)), Failure p, Some (Success (_, _, mk2)) when
                isDistanceMatchKind mk1 && isDistanceMatchKind mk2 -> findStation 3 restOfWikiStations p
            | Some (Success (_, _, mk1)), Failure p, Some (Failure _) when isDistanceMatchKind mk1 ->
                findStation 3 restOfWikiStations p
            | _ -> curr)
    |> removeDoubleWkStations

let private mapReplacements
    (route: RouteInfo)
    (wikiStations: WkOpPointOfRoute [])
...
let private checkDbDataInWikiData
    (route: RouteInfo)
    (wikiStations: WkOpPointOfRoute [])
    (dbStations: DbOpPointOfRoute [])
    =
...
let private compareStations
    ...
    let resultsOfMatch =
        if wikiStations.Length > 0 && dbStations.Length > 0 then
            checkDbDataInWikiData routeInfoMatched wikiStations dbStations
        else
            Array.empty
...
let private findRouteInfoInTemplatesWithParameter (templates: seq<Template>) title showDetails =
    ...
    | Some routeInfos ->
        if routeInfos.Length = 0 then
            printResultOfRoute showDetails (createResult title 0 0 Types.ResultKind.RouteParameterNotParsed)
            ...
            (fun route ->
                printResultOfRoute
                    ...
                    (createResult
                        ...
                        Types.ResultKind.RouteIsNoPassengerTrain)
        ...
        (fun route ->
            ...
            let wikiStations =
                if dbStations.Length > 0 then
                    filterStations routeMatched stationsOfInfobox
                else
                    Array.empty
