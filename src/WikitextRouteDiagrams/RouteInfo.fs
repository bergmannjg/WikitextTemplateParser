...
module RouteInfo =
    ...
    module Data =
        ...
        let insert title route value =
            DataAccess.typedCollectionInsert<RouteInfo> collection (toMap title route) value

        let delete title =
            DataAccess.collectionDelete collection (Map.empty.Add("title", title))

        let query title route =
            DataAccess.typedCollectionQuery<RouteInfo> collection (toMap title route)
    ...
    let printRouteInfo showDetails (routeInfo: RouteInfo) =
        ...
        Data.insert routeInfo.title routeInfo.nummer routeInfo
        |> ignore
    ...
    let showRouteInfoResults () =
        let results =
            Serializer.Deserialize<RouteInfo []>(Data.queryAll ())
    ...
    let private findBsHeaderInTemplates templates =
        match Templates.findTemplateParameterString templates "BS-header" "" with
        | Some value ->
    ...
    let findRouteInfoInTemplates templates title =
        ...
        let kbsTemplates =
            Templates.findTemplateParameterTemplates templates "BS-daten" "KBS"

        let railwayGuide =
            if kbsTemplates |> Seq.isEmpty then
                match Templates.findTemplateParameterString templates "BS-daten" "KBS" with
                | Some value when not (System.String.IsNullOrEmpty value) -> Some(normalizeRailwayGuide value)
                | _ -> None
            else
                match Templates.findTemplateParameterString kbsTemplates "Kursbuchlink" "Nummer" with
                | Some value when not (System.String.IsNullOrEmpty value) -> Some(normalizeRailwayGuide value)
                | _ -> None

        match Templates.findTemplateParameterString templates "BS-daten" "STRECKENNR"
