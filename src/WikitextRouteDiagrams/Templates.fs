...
let findTemplateParameterString (templates: seq<Template>) (templateName: string) (parameterName: string) =
    ...
    |> Seq.map
        (fun p ->
            ...
            | Composite (n, v) when
                ("DE-" + parameterName = n)
                || parameterName = n && v.Length > 0 -> Some(concatCompositeStrings v)
            | _ -> None)
    |> Seq.choose id
    |> Seq.tryExactlyOne
    |> Option.bind
...
let findTemplateParameterTemplates (templates: seq<Template>) (templateName: string) (parameterName: string) =
    ...
    |> Seq.map
        (fun p ->
            ...
            | Composite (n, v) when
                ("DE-" + parameterName = n)
                || parameterName = n && v.Length > 0 -> Some(findCompositeTemplate v)
            | _ -> None)
    |> Seq.choose id
    |> Seq.collect id
...
let textOfLink (link: Link) =
    match link with
    | (t, n) ->
        if not (System.String.IsNullOrEmpty(n)) then
            n.Trim()
        else
            t.Trim()
...
let forallStringsInParameterStringNameList
    (strings: seq<string>)
    ...
    |> Seq.forall
        (fun name ->
            ...
            |> Seq.exists
                (fun t ->
                    match t with
                    | Parameter.String (n, _) when (chooser n name) -> true
                    | _ -> false))

let existsParameterStringNameInList
    (strings: seq<string>)
    (chooser: string -> string -> bool)
    (parameters: seq<Parameter>)
    =
...
let existsParameterStringValueInList
    (strings: seq<string>)
    (chooser: string -> string -> bool)
    (parameters: seq<Parameter>)
    =
